...
Chaque carte se répond : **« J'applique »** (le conseil est suivi, la carte ne revient pas), **« Ignorer »** (elle ne revient que si son coût recalculé grimpe d'au moins 50 %), ou **« Déjà arbitré »** — vous avez déjà pesé ce choix en connaissance de cause, hors de l'outil, et vous consignez la raison en une ligne. Une carte arbitrée ne re-litige plus votre décision : elle rejoint la section repliée « Arbitrages rendus », qui garde le compte visible, la raison et la date affichées, et un bouton « Réactiver » pour lever l'arbitrage d'un clic. Limite assumée : l'identité d'un projet est son **chemin** — un projet déplacé ou renommé est un nouveau sujet, sa carte renaît active.
