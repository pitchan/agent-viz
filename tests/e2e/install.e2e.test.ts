...
import { canonicalProjectKey, missingDistFiles } from '../../src/engine/install/paths.js';
...
function runCli(args: string[]): { code: number; stdout: string; stderr: string } {
  const res = spawnSync(process.execPath, [tsxCli, path.join(netgainRoot, 'src', 'engine', 'cli.ts'), ...args], {
    encoding: 'utf8',
    cwd: netgainRoot,
