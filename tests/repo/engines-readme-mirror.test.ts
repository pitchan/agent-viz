...
const lit = (fichier: string) => readFileSync(path.join(ROOT, fichier), 'utf8');
...
test('le README « Requirements » cite mot pour mot la plage engines.node', () => {
  ...
  // Assert
  expect(requirements.includes('`' + plage + '`'), `README « Requirements » ne cite pas \`${plage}\` :\n${requirements}`).toBeTruthy();
});
...
test('package-lock.json recopie la plage engines.node du paquet racine', () => {
  ...
  // Assert
  expect(racineDuLock.engines.node).toBe(plage);
});
