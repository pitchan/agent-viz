...
import * as r2 from '../../src/server/observatory/rules/r2-unused-mcp.ts';
import type { Session, ConfigItem } from '../../src/server/observatory/rules/types.ts';
...
function session(id: string, { project = 'F--proj', cwd = 'F:/DEV/proj', byTool = {}, toolsAppeared = 0,
...
const userMcp = (name: string): ConfigItem => ({ kind: 'mcp', name, scope: 'user', detail: { transport: 'stdio' } });
const projectMcp = (name: string, key: string): ConfigItem => ({ kind: 'mcp', name, scope: `project:${key}`, detail: { transport: 'stdio' } });
...
test('a user-scope server never called fires, priced on measured tool-block churn', () => {
  ...
  expect(recs[0]!.ruleId).toBe('R2');
  expect(recs[0]!.subject).toBe('mdb-explorer@user');
  expect(recs[0]!.confidence).toBe('fait');
  expect(recs[0]!.costBasis).toBe('jetons-mesures');
  expect(recs[0]!.estimatedCostUsd).toBe(2);
  expect(recs[0]!.evidence.loadedSessions).toBe(3);
  expect(recs[0]!.evidence.usedSessions).toBe(0);
  expect(recs[0]!.evidence.inventorySnapshot).toBe(true);
  expect(recs[0]!.action.includes('à tester')).toBeTruthy();
  expect(recs[0]!.evidence.projects, 'R2 names the projects its loaded sessions belong to (needed by the P3 pointer)').toEqual(['F--proj']);
});
...
test('a project-scope server is only loaded in the sessions of that project', () => {
  ...
  expect(recs.length).toBe(1);
  expect(recs[0]!.evidence.loadedSessions).toBe(2);
});
...
test('a server whose tools are named with underscores is matched correctly', () => {
  ...
  expect(recs, 'used in 100 % of sessions — must not fire').toEqual([]);
});
...
test('with no measurable tool-block churn the recommendation costs zero, and still lists', () => {
  ...
  expect(recs.length).toBe(1);
  expect(recs[0]!.estimatedCostUsd).toBe(0);
});

test('non-MCP config items and empty session lists are ignored', () => {
  expect(r2.evaluate({ sessions: [session('s1')],
    configItems: [{ kind: 'skill', name: 'pdf', scope: 'user', detail: {} }] })).toEqual([]);
  expect(r2.evaluate({ sessions: [], configItems: [userMcp('x')] })).toEqual([]);
});
