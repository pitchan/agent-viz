...
// est affichée.
// ---------------------------------------------------------------------------
import { tokensApplyTo } from '../../src/web/viz-state.ts';

test('un instantané de jetons ne s applique qu à la session affichée', () => {
  expect(tokensApplyTo('29fa8542', '29fa8542')).toBe(true);
  expect(tokensApplyTo('29fa8542', '97ad165d')).toBe(false);
});

test('tant que la session affichée est inconnue, aucun instantané ne s applique', () => {
  // Sinon le dernier instantané rejoué l'emporte : la pastille montre une autre session.
  expect(tokensApplyTo(null, '97ad165d')).toBe(false);
});
