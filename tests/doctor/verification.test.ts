// La queue non vérifiée par session : fusion inter-agents par horodatage, dédup
// d'usage par message.id, une édition en erreur n'a rien modifié, un événement
// sans horodatage est compté — jamais classé au hasard.
//
// Un `.test.ts` parce que ce fichier emploie l'API de vitest (`import { test } from
// 'vitest'`) : l'extension dit le régime (ARCHITECTURE.md § 9).
