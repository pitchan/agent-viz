...
//
// This is a DISPLAY rule and it is deliberately not in the watchdog detector
// (src/engine/watchdog/detector.ts). The watchdog's job is to record what
// happened, with the real time it happened; a detector that refused to see the
// last hour could not feed a journal. The badge's job is narrower — say what
// is wrong NOW — so it, and it alone, applies the cut.
//
