...
import type { Recommendation, Rule, Session, SessionReport } from './rules/types.ts';

const cwdOfReport = (report: SessionReport | null | undefined): string | null => {
  const cwd = report?.cwd;
  return typeof cwd === 'string' && cwd !== '' ? cwd : null;
};

const cwdOf = (session: Session | null | undefined): string | null => cwdOfReport(session?.report);
...
const displayPath = (p: string): string => p.replace(/^([a-z]):/, (_, d: string) => `${d.toUpperCase()}:`);
...
function projectResolver(sessions: Session[] | undefined): (slug: string) => string {
  ...
  for (const s of sessions ?? []) {
    ...
    if (existing === undefined || shown < existing) byFolder.set(folder, shown);
  ...
  for (const [slug, byFolder] of variants) {
    ...
    if (byFolder.size === 1) {
      const [only] = byFolder.values();
      if (only !== undefined) paths.set(slug, only);
    }
  }
  return (slug: string) => paths.get(slug) ?? slug;
}
