...
**Un seul exécuteur, un seul arbre de tests dans 140 fichiers.**

```
find tests -name "*.test.ts" | wc -l   → 140
npx vitest run                          → tous passés, 140 fichiers
```

vitest est le seul exécuteur (`include: tests/**/*.test.ts` de
`vitest.config.mts`), et 140 `.test.ts` en sont l'unique dialecte : tous
