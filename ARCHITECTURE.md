...
**Un seul exécuteur, un seul arbre de tests dans 187 fichiers.**

```
find tests -name "*.test.ts" | wc -l   → 187
npx vitest run                          → tous passés, 187 fichiers
```

vitest est le seul exécuteur (`include: tests/**/*.test.ts` de
`vitest.config.mts`), et 187 `.test.ts` en sont l'unique dialecte : tous
