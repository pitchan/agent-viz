...
**Un seul exécuteur, un seul arbre de tests dans 201 fichiers.**

```
find tests -name "*.test.ts" | wc -l   → 201
npx vitest run                          → tous passés, 201 fichiers
```

vitest est le seul exécuteur (`include: tests/**/*.test.ts` de
`vitest.config.mts`), et 201 `.test.ts` en sont l'unique dialecte : tous
