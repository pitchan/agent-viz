...
**Un seul exécuteur, un seul arbre de tests dans 191 fichiers.**

```
find tests -name "*.test.ts" | wc -l   → 191
npx vitest run                          → tous passés, 191 fichiers
```

vitest est le seul exécuteur (`include: tests/**/*.test.ts` de
`vitest.config.mts`), et 191 `.test.ts` en sont l'unique dialecte : tous
