...
**Un seul exécuteur, un seul arbre de tests dans 198 fichiers.**

```
find tests -name "*.test.ts" | wc -l   → 198
npx vitest run                          → tous passés, 198 fichiers
```

vitest est le seul exécuteur (`include: tests/**/*.test.ts` de
`vitest.config.mts`), et 198 `.test.ts` en sont l'unique dialecte : tous
