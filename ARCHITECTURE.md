...
**Un seul exécuteur, un seul arbre de tests dans 199 fichiers.**

```
find tests -name "*.test.ts" | wc -l   → 199
npx vitest run                          → tous passés, 199 fichiers
```

vitest est le seul exécuteur (`include: tests/**/*.test.ts` de
`vitest.config.mts`), et 199 `.test.ts` en sont l'unique dialecte : tous
