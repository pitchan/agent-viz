...
**Un seul exécuteur, un seul arbre de tests dans 181 fichiers.**

```
find tests -name "*.test.ts" | wc -l   → 181
npx vitest run                          → tous passés, 181 fichiers
```

vitest est le seul exécuteur (`include: tests/**/*.test.ts` de
`vitest.config.mts`), et 181 `.test.ts` en sont l'unique dialecte : tous
