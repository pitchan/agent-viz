...
**Un seul exécuteur, un seul arbre de tests dans 184 fichiers.**

```
find tests -name "*.test.ts" | wc -l   → 184
npx vitest run                          → tous passés, 184 fichiers
```

vitest est le seul exécuteur (`include: tests/**/*.test.ts` de
`vitest.config.mts`), et 184 `.test.ts` en sont l'unique dialecte : tous
