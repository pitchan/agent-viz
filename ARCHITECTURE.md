...
**Un seul exécuteur, un seul arbre de tests dans 190 fichiers.**

```
find tests -name "*.test.ts" | wc -l   → 190
npx vitest run                          → tous passés, 190 fichiers
```

vitest est le seul exécuteur (`include: tests/**/*.test.ts` de
`vitest.config.mts`), et 190 `.test.ts` en sont l'unique dialecte : tous
