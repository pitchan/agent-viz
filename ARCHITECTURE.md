...
**Un seul exécuteur, un seul arbre de tests dans 183 fichiers.**

```
find tests -name "*.test.ts" | wc -l   → 183
npx vitest run                          → tous passés, 183 fichiers
```

vitest est le seul exécuteur (`include: tests/**/*.test.ts` de
`vitest.config.mts`), et 183 `.test.ts` en sont l'unique dialecte : tous
