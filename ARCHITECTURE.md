...
**Un seul exécuteur, un seul arbre de tests dans 197 fichiers.**

```
find tests -name "*.test.ts" | wc -l   → 197
npx vitest run                          → tous passés, 197 fichiers
```

vitest est le seul exécuteur (`include: tests/**/*.test.ts` de
`vitest.config.mts`), et 197 `.test.ts` en sont l'unique dialecte : tous
