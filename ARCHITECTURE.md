...
**Un seul exécuteur, un seul arbre de tests dans 127 fichiers.**

```
npx vitest run     → tous passés, 127 fichiers
```
...
|---|---|---|
| CommonJS + ESM | 41 `.test.cjs` + 61 `.test.mjs` | `node:test` |
...
**Les 102 fichiers en `node:test` passent par un pont** (`test-support/bridge/`),
...
```
grep -rlE "(require\(|from )['\"]node:test['\"]" tests | wc -l   → 102
```
