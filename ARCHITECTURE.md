...
**Un seul exécuteur, un seul arbre de tests dans 205 fichiers.**

```
find tests -name "*.test.ts" | wc -l   → 205
npx vitest run                          → tous passés, 205 fichiers
```

vitest est le seul exécuteur (`include: tests/**/*.test.ts` de
`vitest.config.mts`), et 205 `.test.ts` en sont l'unique dialecte : tous
