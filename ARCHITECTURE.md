...
**Un seul exécuteur, un seul arbre de tests dans 200 fichiers.**

```
find tests -name "*.test.ts" | wc -l   → 200
npx vitest run                          → tous passés, 200 fichiers
```

vitest est le seul exécuteur (`include: tests/**/*.test.ts` de
`vitest.config.mts`), et 200 `.test.ts` en sont l'unique dialecte : tous
