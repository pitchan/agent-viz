...
**Un seul exécuteur, un seul arbre de tests dans 189 fichiers.**

```
find tests -name "*.test.ts" | wc -l   → 189
npx vitest run                          → tous passés, 189 fichiers
```

vitest est le seul exécuteur (`include: tests/**/*.test.ts` de
`vitest.config.mts`), et 189 `.test.ts` en sont l'unique dialecte : tous
