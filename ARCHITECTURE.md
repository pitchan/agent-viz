...
**Un seul exécuteur, un seul arbre de tests dans 185 fichiers.**

```
find tests -name "*.test.ts" | wc -l   → 185
npx vitest run                          → tous passés, 185 fichiers
```

vitest est le seul exécuteur (`include: tests/**/*.test.ts` de
`vitest.config.mts`), et 185 `.test.ts` en sont l'unique dialecte : tous
