...
**Un seul exécuteur, un seul arbre de tests dans 186 fichiers.**

```
find tests -name "*.test.ts" | wc -l   → 186
npx vitest run                          → tous passés, 186 fichiers
```

vitest est le seul exécuteur (`include: tests/**/*.test.ts` de
`vitest.config.mts`), et 186 `.test.ts` en sont l'unique dialecte : tous
