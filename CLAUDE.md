...
- Concrètement dans ce repo : un fichier de `src/server/routes.ts` ne fait pas de logique métier ; un hook ne fait pas de routing ; le parsing d'événements ne fait pas d'I/O fichier.
...
- Un précédent existe déjà dans le repo : `src/server/routes.ts` table déclarative — réutilise ce pattern.
...
- **Garde-fou mesuré de taille** : `tests/repo/file-size-budget.test.mjs` échoue si un fichier de `src/` ou de `bin/` dépasse 450 lignes sans entrée justifiée dans sa liste `LISTE_BLANCHE` — découper d'abord, justifier sinon. La liste est un cliquet : une entrée repassée sous le seuil doit en sortir.
- **Garde-fou mesuré des commentaires** : un commentaire dit POURQUOI le code est ainsi, au présent, en trois lignes au plus — jamais un récit, ni un renvoi à un plan, une étape, une date ou un document numéroté.
  - `tests/repo/no-narrative-comments.test.mjs` échoue si un commentaire de `src/`, `bin/`, `tests/`, `test-support/` ou `vitest.config.mts` porte un de ces marqueurs sans entrée justifiée dans sa liste `LISTE_BLANCHE`, ou si un bloc de commentaire dépasse 30 lignes.
  - Cette liste blanche n'admet qu'une donnée datée, jamais un récit : une entrée qui ne couvre plus rien doit en sortir.
