...
- **Garde-fou mesuré des commentaires** : un commentaire dit POURQUOI le code est ainsi, au présent, en trois lignes au plus — jamais un récit, ni un renvoi à un plan, une étape, une date ou un document numéroté. `tests/repo/no-narrative-comments.test.mjs` échoue si un commentaire de `src/`, `bin/`, `tests/`, `test-support/` ou `vitest.config.mts` porte un de ces marqueurs sans entrée justifiée dans sa liste `LISTE_BLANCHE`, ou si un bloc de commentaire dépasse 30 lignes. Cette liste blanche n'admet qu'une donnée datée, jamais un récit : une entrée qui ne couvre plus rien doit en sortir.
