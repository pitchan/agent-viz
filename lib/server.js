...
const { demarrerChienDeGarde } = require('./server/watchdog');
...
killOldServer().then(async () => {
  ...
  // Le chien de garde : instance, rattrapage de ce qui s'est passe serveur
  // eteint, puis battement. La sequence et son ordre vivent dans
  // server/watchdog, ou ils sont essayables ; ce qui se decide ICI est ce qui
  // n'appartient qu'au serveur — le dossier d'evenements, pris a `DIR`, qui en
  // est la seule definition faisant autorite, et le vrai flux SSE.
  await demarrerChienDeGarde({ dir: DIR, diffuser: broadcastSSE });
  startServer();
});
