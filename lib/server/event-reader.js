  ...
  try {
    ...
    for (const line of lines) {
      try {
        ...
        // Le chien de garde voit le meme flot que le canevas, mais il en tire
        // autre chose : ce qui merite d'etre consigne. `onEvent` rend
        // uniquement les faits inedits — un fichier relu ne rediffuse rien.
        // Apres la diffusion de l'evenement, jamais avant : ce que le serveur
        // annonce doit deja etre au journal, et une alerte ne precede pas
        // l'evenement qui l'a produite.
        const wd = getWatchdogService();
        if (wd) for (const alert of wd.onEvent(evt)) broadcastSSE({ type: 'alert', alert });
      } catch {}
    }
