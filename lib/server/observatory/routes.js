  ...
  return [
    {
      ...
      handler: bind(async (_req, res, url, service) => {
        const summary = await service.summary({ since: url.searchParams.get('since') || undefined });
    ...
    },
    {
      ...
      handler: bind(async (_req, res, _url, service) => {
        service.scan().catch(err => console.error('[observatory] scan failed:', err.message));
