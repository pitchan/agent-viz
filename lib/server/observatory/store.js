...
function openStore(dbPath) {
  ...
  const insertSession = db.prepare(`
    INSERT INTO sessions (id, project, transcript_path, file_mtime, file_size, scan_version,
      started_at, ended_at, model_main, net_tokens, cost_usd, cost_complete, report_json)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)
  ...
  const upsertRec = db.prepare(`
    INSERT INTO recommendations (rule_id, subject, created_at, updated_at, last_seen_at, title,
      category, confidence, estimated_cost_usd, cost_basis, evidence_json, action, status, cost_at_status_usd)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?, 'new', NULL)
  ...
  return {
    ...
    upsertSession(s) {
      insertSession.run(s.id, s.project, s.transcriptPath, s.fileMtime, s.fileSize,
        s.scanVersion, s.startedAt, s.endedAt, s.modelMain, s.netTokens, s.costUsd,
        s.costComplete ? 1 : 0, s.reportJson);
    },

    listSessions({ project, since } = {}) {
      const where = [];
      const args = [];
    ...
    upsertRecommendations(recs, now) {
      for (const r of recs) {
        upsertRec.run(r.ruleId, r.subject, now, now, now, r.title, r.category, r.confidence,
          r.estimatedCostUsd, r.costBasis, JSON.stringify(r.evidence), r.action);
      }
    },
