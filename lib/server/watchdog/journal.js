...
// Les horodatages d'acquittement franchissent une frontiere HTTP (tache 8), ou
// tout parametre de requete arrive en chaine. `Number('')` et `Number(null)`
// valant 0, la conversion est restreinte aux chaines non vides : sans ca, une
// valeur absente deviendrait un horodatage a l'epoque Unix au lieu d'un refus
// franc. Rend null quand il n'y a pas d'horodatage utilisable.
...
function createJournal({ filePath = DEFAULT_PATH, now = Date.now } = {}) {
  ...
  return {
    ...
    appendAck(id, createdAt, at) {
      ...
      write({ kind: 'ack', id, createdAt: quand, at: vu });
    },
