...
//
// `dir` et `diffuser` sont donc injectes — ce module ne connait ni le dossier
// des evenements (`session-index` en fait autorite) ni le flux SSE.
// `cadenceMs` l'est aussi, avec la valeur du client d'aujourd'hui
...
async function demarrerChienDeGarde({ dir, diffuser, cadenceMs = 5_000, init } = {}) {
  await initWatchdog(init);
  let relus = 0;
  try {
    relus = await runCatchUp(dir);
  } catch (err) {
  ...
  const battement = setInterval(() => {
    // Relu a chaque battement, pas capture une fois : le service peut ne pas
    // encore exister, ou ne jamais exister (installation abimee). Sans cette
    // garde, l'exception partirait d'un rappel de minuteur, ou personne ne
    // l'attrape — et une exception non attrapee tue le processus.
    const wd = getWatchdogService();
    if (wd) for (const alert of wd.tick()) diffuser({ type: 'alert', alert });
  }, cadenceMs);
