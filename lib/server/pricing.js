...
// call getPrice() / computeCost() and don't know the source.
...
// (tests/unit/pricing-engine-mirror.test.js keeps the two in lockstep) —
// applies before the engine loads at boot and whenever it is absent. Numbers
...
// local model = one line here (same philosophy as the dated periods: one line
// per fact). Mirror of netgain src/core/pricing.ts ZERO_COST.
...
// Strip provider-specific prefixes/suffixes ("anthropic.", "us.anthropic.",
// "bedrock/", "-v1:0", "-v1", "-20251101", "@20250929", "[1m]") so a single
// canonical id resolves prices regardless of which transport or regional
// router the model was called through. The regional-router prefix
// (global|us|eu|au).anthropic. must be stripped before the plain
// "anthropic." prefix so "us.anthropic.claude-opus-4-7" doesn't fall through
// as a distinct, un-normalized id — that was the root cause of most of the
// 109 false "modele-nouveau" alerts against the real LiteLLM feed.
...
function litellmDrift(json, at) {
  ...
  for (const [k, v] of Object.entries(json)) {
    ...
    if (!local) {
      ...
      if (isNew) drifts.push({ model: canonical, kind: 'modele-nouveau', litellm: upstream, embedded: null });
      continue;
    }
