...
// crash. Cache pricing uses the 5min rate (default Anthropic tier); 1h cache
// and >200k tiered pricing are not modeled here.
...
function ingestLitellm(json) {
  ...
  for (const [k, v] of Object.entries(json)) {
    ...
    next[canonical] = {
      ...
      cacheCreate: v.cache_creation_input_token_cost || 0,
      cacheRead: v.cache_read_input_token_cost || 0,
      maxInput: v.max_input_tokens || 200_000,
...
function startPricingRefresh() {
  ...
  refreshTimer = setInterval(() => loadPricing().catch(() => {}), REFRESH_MS);
  refreshTimer.unref();
}
