...
// token. Used at boot before the first fetch resolves and as a permanent
// safety net if the LiteLLM URL is unreachable. Numbers must be kept aligned
// with Anthropic's public rate card. These are the CURRENT rates; past rate
// periods live in PRICE_HISTORY below.
...
// Dated tariff memory: rate periods that PRECEDE the current rates, sorted by
// ascending `until`. A period applies to messages timestamped STRICTLY before
// its `until` (ISO UTC, lexicographic compare — both sides are zero-padded).
// Rate fields only; label/maxInput are inherited from the current entry. A
// model absent from this map never changed tariff. LiteLLM only carries
// current prices, so this map is static by design.
const PRICE_HISTORY = Object.freeze({
  // Sonnet 5: intro rate 2/10 $ per MTok through 2026-08-31 (Anthropic
  // announcement), sticker 3/15 afterwards.
...
// Deliberate zero-cost models: not billable BY NATURE (harness artifact,
// local inference). A zero from this list is a WANTED zero — the "never a
// silent zero" rule targets unknown models; these are known, at $0. A new
// local model = one line here (same philosophy as PRICE_HISTORY). Mirror of
// netgain src/core/pricing.ts ZERO_COST.
const ZERO_COST = new Set([
  '<synthetic>',        // Claude Code harness artifact — no API call
  'ministral-3:latest', // local Ollama model (S8 trials) — no API billing
]);
...
// "-v1:0", "-20251101", "@20250929", "[1m]") so a single canonical id resolves
// prices regardless of which transport the model was called through.
...
// instant; omitted means "now". The returned object keeps the current entry's
// non-rate fields (maxInput, label) — only the rates travel back in time.
function getPrice(id, at) {
  ...
  const canonical = prices[id] ? id : normalizeId(id);
  const history = PRICE_HISTORY[canonical];
  if (history) {
...
// costs aren't invisible.
//
// Cache pricing splits by tier when the API exposes the breakdown:
//   usage.cache_creation.ephemeral_5m_input_tokens → p.cacheCreate (1.25× input)
//   usage.cache_creation.ephemeral_1h_input_tokens → 2× input (Anthropic rate)
// Without the breakdown (older transcripts / providers that don't expose it),
// the total is billed at the 5min rate — matches legacy behavior.
// >200k tiered pricing is still not modeled here.
const _loggedUnknownModels = new Set();
function computeCost(usage, modelOrPrice, at) {
  if (!usage) return 0;
  const p = (modelOrPrice && typeof modelOrPrice === 'object')
    ? modelOrPrice
    : getPrice(modelOrPrice, at);
  if (!p) {
    if (typeof modelOrPrice === 'string') {
      const canonical = normalizeId(modelOrPrice);
      // Wanted zero: known non-billable model, no warning — the warning is
      // reserved for models whose tariff we do NOT know.
      if (canonical && ZERO_COST.has(canonical)) return 0;
      if (modelOrPrice && !_loggedUnknownModels.has(modelOrPrice)) {
  ...
  const cc1h = (cc && cc.ephemeral_1h_input_tokens) || 0;
  // When the breakdown is present, trust it exactly. When absent, treat the
  // whole total as 5min — keeps cost monotonic and never NaN.
  const cc5m = cc
...
function ingestLitellm(json) {
  ...
  for (const [k, v] of Object.entries(json)) {
    ...
    // Match Claude entries across all transports (fable/mythos = Claude 5 tier).
