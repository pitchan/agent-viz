...
function ask({ question, options, initial, io }) {
  return new Promise((resolve, reject) => {
    ...
    const cleanup = () => {
      ...
      // Symmetric to input.resume() in setup. Without these, stdin stays in
      // flowing mode with internal listeners attached by emitKeypressEvents,
      // and the event loop refuses to drain — install-hooks would hang on exit.
      // unref() is the belt-and-suspenders fix for Windows raw-mode TTY,
      // where pause() alone has been observed to leave the libuv tty handle
      // active. Together they guarantee the process can exit naturally.
