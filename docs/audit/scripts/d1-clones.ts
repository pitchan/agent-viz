...
import { tokenize } from './lib/tokens.ts';
...
type Occurrence = { path: string; zone: string; i: number; lineStart: number; lineEnd: number; key: string };
type CloneGroup = {
  empreinte: string;
  jetons: number;
  sites: { path: string; zone: string; lineStart: number; lineEnd: number }[];
  interZone: boolean;
};

export function findClones(files: SourceFile[]): CloneGroup[] {
  const tokensByFile = new Map<string, ReturnType<typeof tokenize>>();
  ...
  const bySignature = new Map<string, Map<string, Occurrence[]>>();
