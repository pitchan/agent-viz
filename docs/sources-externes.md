...
| `doc/12` | Design « Observatoire » (M1) — spec validée le 2026-07-27 | §7 : les seuils de R2 (50 % / 10 %), R5 (≥ 2) et R6 (5 min, 30 %). Les changer, c'est changer la spec | `src/server/observatory/rules/thresholds.js` — chaque seuil déclare son origine (`'spec'` ou `'calibration'`) ; `tests/unit/observatory-rules-cost.test.js` épingle les valeurs |
